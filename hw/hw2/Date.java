...
class Date {
  ...
  public Date(int month, int day, int year) {
    if (!isValidDate(month, day, year)){
      System.out.println("Date not valid, you ASSHOLE!");
      System.exit(0);
    }
...
month == 10 || month == 12) return 31;
  ...
  public static boolean isValidDate(int month, int day, int year) {
    ...
    if (day > daysInMonth(month, year) || day < 1) return false;    
    return true;
  }
  ...
  public int difference(Date d){
    int dif = 0;
    // if (this.isAfter(d)){
    //   Date temp1 = new Date(this);
    //   Date temp2 = new Date(d);
    // }
    // else{
    //   Date temp2 = new Date(this);
    //   Date temp1 = new Date(d); 
    // }
    if (this.year == d.year && this.month == d.month && this.day == this.day){
      return 0;
    }
    if (this.isAfter(d)){
      ...
      if (this.year > d.year + 1){
        for (int i = d.year + 1; i < this.year; i++){
          dif += 365;
          if (isLeapYear(i)) dif += 1;
        }
        dif += 365 - d.dayInYear();
        if (isLeapYear(d.year)) dif += 1;
        dif += this.dayInYear();
      }
