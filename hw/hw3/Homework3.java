...
public class Homework3 {
  ...
  public static void smoosh(int[] ints) {
    ...
    while(head < ints.length - 1){
      tail = head + 1;
      while(ints[head] == ints[tail] && tail < ints.length - 1) tail++;
      result[result_pt++] = ints[head];
      head = tail;
    }
    //don't forget the last element
    if (ints[head]!=ints[head-1]) result[result_pt] = ints[head];
    for (int ii = 0; ii < ints.length; ii++) {
      ints[ii] = result[ii];
    }
  }
