...
class SibTreeNode extends TreeNode {
  ...
  public void insertChild(Object item, int c) throws InvalidNodeException {
    ...
    if (firstChild == null) {
      ...
      if (c > children()) {
        ((SibTreeNode)(child(children()))).nextSibling = child;
      } else {
        if (c <= 1) {
          SibTreeNode kid = firstChild;
          firstChild = child;
          child.nextSibling = kid;
        }
        else {          
          SibTreeNode kid = (SibTreeNode)child(c-1);
          SibTreeNode temp = kid.nextSibling;
          kid.nextSibling = child;          
  ...
  public void removeLeaf() throws InvalidNodeException {
    ...
    if (parent != null){ // not root
      SibTreeNode remove = parent.firstChild;
      if (remove == this) {
        parent.firstChild = nextSibling;
      }
      else {
        while (remove.nextSibling != null && remove.nextSibling != this){
        remove = remove.nextSibling;
        }
        remove.nextSibling = nextSibling;
      }     
    }    
  }
